...
bool backtrack(vector<vector<char>>& board, const string& targetWord, int i, int j, int k) {
    int n = board.size();
    int m = board[0].size();
    ...
    // Check if the current cell is out of bounds or doesn't match the target character
    ...
    bool found = backtrack(board, targetWord, i + 1, j, k + 1) ||
